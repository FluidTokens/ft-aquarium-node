...
public class PreviewTankTest {
    ...
    private final SpendValidator spendValidator = new SpendValidator(preview());
    ...
    public void deployContract() throws Exception {
        ...
        PlutusScript tankScript = spendValidator.getPlutusScript();

        var adminAddress = new Account(network, PreviewTestConstants.MNEMONIC_ADMIN);

        Tx createRefInputTx = new Tx()
                .from(adminAddress.baseAddress())
                .payToAddress("addr_test1qp7nk0p4y8tjdn2eqzff9hydx3d3a0445lu05j7n94fmuz9ge95zfe83krrk894fdqgj0mduq9w5c32vwn6qat48x39qmz5wnj", List.of(Amount.ada(1)), tankScript)
                .withChangeAddress(adminAddress.baseAddress());

        quickTxBuilder.compose(createRefInputTx)
                .feePayer(adminAddress.baseAddress())
    ...
    public void setupPayment() throws Exception {
        ...
        var account = new Account(preview(), MNEMONIC_USER);
...
//        allowedToken.toPlutusData();
        ...
        var tankOwnerAddress = AddressProvider.getEntAddress(spendValidator.getPlutusScript(), preview());
